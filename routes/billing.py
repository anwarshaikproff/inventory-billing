...
from database.db import query_db, execute_db, get_db_status, translate_query, _CHOSEN_DB_TYPE
...
def index():
    ...
    for b in bills:
        try:
            ...
            if 'cash' in pay_type:
                total_cash += price
            elif 'online' in pay_type or 'upi' in pay_type or 'card' in pay_type:
                total_online += price
        except Exception:
            pass
...
def add_bill():
    ...
    try:
        ...
        cost_of_the_item = float(data.get('cost_of_the_item', 0.0))
        item_qty = float(data.get('item_qty', 1.0))
        
        # Auto-calculate item price
        item_price = round(cost_of_the_item * item_qty, 2)
        payment_type = data.get('payment_type', 'cash').lower().strip()
        ...
        if not customer_name or not phone_no or not item_name:
            if request.is_json:
                return jsonify({"status": "error", "message": "Customer Name, Phone No, and Item Name are required!"}), 400
            flash("Customer Name, Phone No, and Item Name are required!", "danger")
            return redirect(url_for('billing.index'))

        query = """
            INSERT INTO billing (customer_name, phone_no, item_name, cost_of_the_item, item_qty, item_price, payment_type)
...
def status():
    """Returns current cloud DB status."""
